...
- **หน้าสรุปตามสาขา**: รายงานสรุปปัญหาของแต่ละบริษัท/สาขา ย้อนหลังได้ตามช่วงวันที่ที่เลือก (ไม่จำกัดแค่วันเดียว) กรองดูเฉพาะสาขาใดสาขาหนึ่งได้ แต่ละการ์ดมีปุ่ม **"คัดลอกข้อความ"** และปุ่ม **"🖨️ พิมพ์รายงาน"** ที่เปิดหน้าต่างพิมพ์ของเบราว์เซอร์ (เลือก "บันทึกเป็น PDF" ได้ด้วย) ตัดส่วนเมนู/ปุ่มต่างๆ ออกให้เหลือแต่เนื้อหารายงาน
- **หน้าวิเคราะห์ปัญหาซ้ำ**: ใส่คีย์เวิร์ด (คั่นด้วย comma เช่น "กระดาษติด, ไม้กั้นไม่เปิด") จะนับให้ว่าตรงกับ ticket กี่เคส แยกแก้ไขแล้ว/ยังแก้ไม่ได้ ทั้งภาพรวมและแยกทีละคีย์เวิร์ดพร้อมลิงก์เลข ticket เว้นช่วงวันที่ว่างไว้ = ดูทั้งหมดตั้งแต่เริ่มระบบ หรือระบุช่วงวันที่ก็ได้ มีปุ่มคัดลอกและพิมพ์รายงานเหมือนหน้าสรุปตามสาขา
...
- **ฟิลด์บริษัท/สาขา** ตอนเปิด ticket ใหม่ — พิมพ์แล้วมี autocomplete ให้เลือกจากทั้งรายชื่อสาขาใน AnyDesk Directory และชื่อบริษัทที่เคยพิมพ์ไว้ในบริษัทอื่น (พิมพ์ชื่อใหม่ครั้งเดียว ครั้งต่อไปจะขึ้นให้เลือกอัตโนมัติ) แสดงเป็นแท็กสีม่วงบนการ์ด Dashboard ด้วย
  - **หัวข้อ ticket ใส่ชื่อบริษัทให้อัตโนมัติ**: ถ้าเลือก/พิมพ์บริษัทไว้ ไม่ต้องพิมพ์ชื่อบริษัทซ้ำในหัวข้อ พิมพ์แค่ปัญหา (เช่น "เครื่องปริ้นไม่ดูดกระดาษ") ระบบจะตั้งหัวข้อให้เป็น "โรบินสัน ฉะเชิงเทรา : เครื่องปริ้นไม่ดูดกระดาษ" ให้เองตอนสร้าง (ใช้กับ ticket ที่แจ้งมาจากกลุ่ม LINE ด้วยเช่นกัน)
...
| GET | `/api/summary/range?from=YYYY-MM-DD&to=YYYY-MM-DD&company=` | ticket ทั้งหมดในช่วงวันที่ (กรองด้วย `company` ได้) พร้อมบันทึกงาน สำหรับหน้าสรุปตามสาขา |
| GET | `/api/summary/keywords?keywords=a,b&from=&to=` | นับจำนวน ticket ที่ตรงกับแต่ละคีย์เวิร์ด (ค้นหาใน title+description) แยกแก้ไขแล้ว/ยังไม่แก้ `from`/`to` ไม่ระบุ = ทั้งหมดทุกช่วงเวลา |
