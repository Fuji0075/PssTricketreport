...
- **หน้าวิเคราะห์ปัญหาซ้ำ**: ใส่คีย์เวิร์ด (คั่นด้วย comma เช่น "กระดาษติด, ไม้กั้นไม่เปิด") จะนับให้ว่าตรงกับ ticket กี่เคส แยกแก้ไขแล้ว/ยังแก้ไม่ได้ ค้นทั้งหัวข้อ/รายละเอียด/บันทึกในช่องคอมเมนต์ มีให้ดู 2 มุม:
  - **สรุปตามสาขา**: รูปแบบ "ชื่อสาขา = คีย์เวิร์ด = จำนวนครั้ง" พร้อมติดป้าย "เป็นซ้ำบ่อย" ถ้าคีย์เวิร์ดนั้นเกิดที่สาขานั้นตั้งแต่ 2 ครั้งขึ้นไป เห็นชัดว่าสาขาไหนมีปัญหาเดิมซ้ำๆ พร้อมสรุปต่อสาขา: เคสทั้งหมด/แก้ไขแล้ว/ยังไม่แก้/**On Hold** กี่เคส และลิสต์ชื่องานที่ถูกพักไว้ (On Hold) ให้เห็นตรงๆ ว่ามีงานอะไรค้างรอช่างอยู่บ้าง
  - **สรุปตามคีย์เวิร์ด**: รวมทุกสาขา พร้อม **วิธีแก้ที่พบบ่อยที่สุด** (ดึงจากช่องรายละเอียดและบันทึกในคอมเมนต์ จัดอันดับตามความถี่ที่พบ) เผื่อกรณีปัญหาเดิมส่วนใหญ่แก้ด้วยวิธีเดียวกัน — ถ้าตั้งค่า `ANTHROPIC_API_KEY` ไว้ ระบบจะให้ Claude ช่วยรวมกลุ่มข้อความที่พิมพ์ต่างกันแต่ความหมายเดียวกันเข้าด้วยกันก่อนนับด้วย (ถ้าไม่ตั้งค่าไว้ก็ยังใช้งานได้ปกติ แค่นับแบบข้อความตรงกันเป๊ะๆ)

  เว้นช่วงวันที่ว่างไว้ = ดูทั้งหมดตั้งแต่เริ่มระบบ หรือระบุช่วงวันที่ก็ได้ มีปุ่มคัดลอกและพิมพ์รายงานเหมือนหน้าสรุปตามสาขา
...
| GET | `/api/summary/keywords?keywords=a,b&from=&to=` | นับจำนวน ticket ที่ตรงกับแต่ละคีย์เวิร์ด (ค้นหาใน title+description+comment) แยกแก้ไขแล้ว/ยังไม่แก้/On Hold ทั้งแบบรวมและแยกตามสาขา พร้อมวิธีแก้ที่พบบ่อย `from`/`to` ไม่ระบุ = ทั้งหมดทุกช่วงเวลา |
