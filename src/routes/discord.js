...
router.get('/status', (req, res) => {
  res.json({ configured: isDailySummaryConfigured() });
});

// Relay endpoint for the manual "send now" button — keeps the webhook URL
...
router.post('/send', async (req, res) => {
  if (!isDailySummaryConfigured()) {
    return res.status(400).json({ error: 'DISCORD_DAILY_SUMMARY_WEBHOOK_URL is not configured' });
  }
